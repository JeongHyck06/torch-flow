...
req = urllib.request.Request(f"http://127.0.0.1:8799/api/scalars", headers={"Authorization": f"token {sess.token}"})
print("기록된 스칼라:", len(json.loads(urllib.request.urlopen(req).read())["scalars"]), "개")
print("URL:", sess.url)
sess.close()
